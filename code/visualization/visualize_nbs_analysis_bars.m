...
components = [68]; % 19, 68
thresholds = [4]; % 3.1, 4
edge_plots = {'perc_sig_FC'}; %'count_sig_FC', 'perc_sig_FC', 'percarea_sig_FC'}; 
method = 'Extent';
    ...
    for comp = 1:length(components)
        ...
        for e = 1:length(edge_plots)
            ...
            for c = 1:length(comparisons)
                ...
                if strcmp(comparison, 'postic-prem') & nComp == 68
                    continue
                end
                
                contrast_type = contrast_types{c};
        
                matrix = triu(round(edge_data(:,:,c),1));

                % Sort the elements of the matrix in ascending order
                [sortedValues, linearIndices] = sort(matrix(:), 'descend');
                
                % Get cumulative sum larger than threshold percentage 
                sum_values = cumsum(sortedValues);
                thresh_ind = find(sum_values > thresh);
                ind_max = thresh_ind(1);
                ...
                sig_networks = indexes(1:ind_max, :);
                ...
                total_conn = (nr_areas*nr_areas-nr_areas)/2;
                possible_conn = tril(adj_matrix,0);
                real_conn = sum(possible_conn(:));
                perc_conn = 100*(real_conn/total_conn);

                figure('pos', [50, 50, 500 150]);
%                 subplot(3,1,[1 2])
...
%                 set(gca, 'XTickLabel', '')
%                 set(gca, 'YTickLabel', '')
%                 title([comparison, ' - ', num2str(real_conn), ' - ', num2str(perc_conn), '%'])
%                 
                %subplot(3,1,3)
                bar(sortedValues(1:ind_max), ind_max/8, 'FaceColor', [0.6 0.6 0.6], 'EdgeColor', 'none');
                set(gca, 'XTickLabel', connection_labels)
                set(gca, 'XTick', 1:ind_max)
                set(gca, 'FontSize', 11)
                set(gca, 'XTickLabelRotation', 20)
                ylim([0 1.1*sortedValues(1)])
                xlim([0.5 ind_max+0.5])
                ylabel(edge_data_ylabel)
                ylim([0 46])
                %title([comparison, ' - ', num2str(real_conn), ' - ', num2str(perc_conn), '%'])

                print([fig_path, '/nbs_', num2str(nComp),'PCs_FCmatrix-sigbars_', plot_name, '_', comparison, '_contrast', num2str(contrast_type), '_method-',method, '_cluster-', num2str(round(threshold))], '-dpng')
